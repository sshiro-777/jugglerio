...
const CriticalFewList: React.FC<CriticalFewListProps> = ({ tasks, projects, onSelectTask, activeTimer }) => {
      ...
      <div className="space-y-4">
        {sortedTasks.length > 0 ? (
          sortedTasks.slice(0, 3).map(task => { // Limiting to top 3 for a cleaner look
            ...
            const isTiming = activeTimer?.taskId === task.task_id;
            return (
              <button 
                key={task.task_id} 
                onClick={() => onSelectTask(task)}
                className={`w-full text-left bg-dark-bg p-4 rounded-lg flex items-center justify-between gap-4 transition-all hover:bg-slate-800 ${isTiming ? 'ring-2 ring-brand-primary' : ''}`}
              >
                <div className="flex items-center gap-3 flex-grow min-w-0">
                  {isTiming && <TimerIcon className="w-5 h-5 text-brand-primary animate-pulse flex-shrink-0" />}
